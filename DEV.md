...
Create a new plain text file in the root directory (same folder as this document) called `.env`, with `UPLOAD_LIMIT=20` and  `IMGUR_CLIENT_ID=idGoesHere` as the only lines. You can do it manually or with this command:

```
printf "UPLOAD_LIMIT=20\nIMGUR_CLIENT_ID=idGoesHere" > .env
```
...
| `IMGUR_CLIENT_ID` | 740273babac99b0 | This is needed to ensure that you can make full use of the uploading api for Imgur. There have been times where it works without one, but it gets rate limited way quicker, or just doesn't work. Makee sure to set this if you don't want to run into problems uploading with imgur! |
